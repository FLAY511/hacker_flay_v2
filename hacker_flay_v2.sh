#!/usr/bin/env bash
# HACKER_FLAY v2 — Full toolkit (OSINT / JSO / Utilities / Dark Web)
# Banner: Cyber Skull (seram). Tekan Enter setelah membaca instruksi.
# Gunakan hanya untuk tujuan legal/authorized testing & education.
...
osint_menu(){
  ...
  case "$O" in
    ...
    6)
      ...
      ;;
    7)
      read -rp "Domain/URL: " d
      curl -Is "$d" 2>/dev/null || echo "Gagal ambil header"
      ;;
    8)
      read -rp "Email: " e
      echo "Buka: https://haveibeenpwned.com/account/$e"
      ;;
    9)
...
jso_menu(){
  ...
  if [[ "$L" != "lanjutkan" ]]; then echo "Dibatalkan"; return; fi
...
menu_lain(){
  ...
  read -rp "Pilih: " U
  case "$U" in
    1) read -rp "Kota: " k; curl -s "wttr.in/${k}?format=3" ;;
    2) echo "Ketik 'quit' untuk keluar"; while read -rp "expr> " e; do [[ "$e" == "quit" ]] && break; echo "$e" | bc; done ;;
    3) arr=("DarkGhost" "CyberNinja" "ShadowX" "NullByte" "RootKing"); echo "Nama: ${arr[$RANDOM % ${#arr[@]}]}" ;;
    4) if command -v termux-open-url >/dev/null 2>&1; then termux-open-url "https://chat.openai.com"; else echo "Buka: https://chat.openai.com"; fi ;;
    5) read -rp "Teks: " t; echo "Encode: $(echo -n "$t" | base64)"; echo "Decode: $(echo -n "$t" | base64 -d 2>/dev/null)" ;;
    6) read -rp "Teks: " t; echo "MD5: $(echo -n "$t"|md5sum|awk '{print $1}')"; echo "SHA256: $(echo -n "$t"|sha256sum|awk '{print $1}')" ;;
    7) openssl rand -base64 12 ;;
    8) curl -s ifconfig.me || echo "Gagal ambil IP publik" ;;
    9) if command -v speedtest-cli >/dev/null 2>&1; then speedtest-cli; elif command -v speedtest >/dev/null 2>&1; then speedtest; else echo "Install speedtest via pkg install speedtest"; fi ;;
    10) cal ;;
    0) return ;;
    *) echo "Pilihan tidak valid"; ;
  esac
...
darkweb_menu(){
  ...
  echo "- Saat memilih, browser akan dibuka ke Hidden Wiki (atau mirror)."
